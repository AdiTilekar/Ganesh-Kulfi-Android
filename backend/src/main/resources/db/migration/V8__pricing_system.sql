...
-- 1. Create retailer tier enum
DO $$ BEGIN
    CREATE TYPE retailer_tier AS ENUM ('BASIC', 'SILVER', 'GOLD', 'PLATINUM');
EXCEPTION
...
-- Map old pricing_tier values to new retailer_tier values
-- Old: VIP, PREMIUM, REGULAR, RETAIL -> New: BASIC, SILVER, GOLD, PLATINUM
UPDATE app_user SET tier = CASE 
    WHEN tier = 'VIP' THEN 'PLATINUM'
    WHEN tier = 'PREMIUM' THEN 'GOLD'
    WHEN tier = 'REGULAR' THEN 'SILVER'
    WHEN tier = 'RETAIL' THEN 'BASIC'
    ELSE 'BASIC'
END;
